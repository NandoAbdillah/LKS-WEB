...
export default function RequestDataValidation() {
    ...
    useEffect(()=> {
        setNamePage("Request Data Validation");

        const fetchCategories = async() => {
             try {
                const response = await httpClient('http://127.0.0.1:8000/api/v1/job_categories', {
                    method : 'GET',
                    token : token
                });
    ...
    return (
        <form className="col-10 mx-auto mb-5" onSubmit={handleSubmit}> 
            <div className="row mb-3">
             <div className="col-6">
                <label className="form-label" htmlFor="jobCategory">Job Category</label>
                <select id="jobCategory" className="form-select form-select-md">
                    <option selected disabled>Choose one job category</option>
                    {
                        jobCategories && jobCategories.map((item, index)=>(
                            <option key={index} ref={jobCategoryIdRef} value={item.id} >{item.job_category}</option>
                        ))
                    }
            ...
            <div className="row mb-3">
              <div className="col-6">
                    <div className="form-floating">
                        <textarea id="jobPositionDescribe" className="form-control" placeholder="Job position sparated with ,(coma)"style={{ 
                            height : '200px'
                         }} ref={jobPositionRef} ></textarea>
                        <label htmlFor="jobPositionDescribe">
                          Job Positions
                        </label>
              ...
              <div className="col-6">
                 <div className="form-floating">
                    <textarea id="workExperienceDescribe" className="form-control" style={{ 
                        height : '200px'
                     }}
                     placeholder="Describe your work experience"
                     ref={jobExperienceRef}
                     ></textarea>
                     <label className="form-label" htmlFor="workExperienceDescribe">Work Experience</label>
                     <div className="form-text">
            ...
            <div className="row mb-3">
              ...
              <label className="form-label" htmlFor="reasonAccepted">Reason Accepted</label>
                <textarea className="form-control" id="reasonAccepted" placeholder="Explain why you should be accepted" style={{ 
                    height : '200px'
                 }}
